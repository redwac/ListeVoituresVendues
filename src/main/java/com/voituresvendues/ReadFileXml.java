...
// nodeList is not iterable, so we are using for loop
            ...
            for (int itr = 0; itr < nodeList.getLength(); itr++)
            {
                ...
                if (node.getNodeType() == Node.ELEMENT_NODE)
                {
                    ...
                    System.out.println("la r�ference de voiteure est : "+ eElement.getElementsByTagName("reference").item(0).getTextContent());
                    System.out.println("la marque de voiture est : "+ eElement.getElementsByTagName("marque").item(0).getTextContent());
                    System.out.println("Le mod�le de voiture est : "+ eElement.getElementsByTagName("modele").item(0).getTextContent());
